...
-- v2.1: replace flat 0.12 total threshold with dimension-gating:
-- a row passes if ANY field scored >= its per-field threshold.
-- Keeps noisy trigram hits out while letting single-axis filters (year, place) through.
...
AS $$
  ...
  final_rows AS (
    SELECT
      s.*,
      -- Pass-gate: at least one field with a decent match, OR year/place exact match

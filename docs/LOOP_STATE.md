...
| B4 | M0: `app_settings` (auto_merge_enabled=false, 450, 0.78) + `matching_runs` + `person_privacy_holds` + `is_person_living()` + pgTAP (plan §7-M0) | matching, db | B1 | TODO | NOTE: `is_person_living()` already shipped in B3 — B4 covers the remaining tables (app_settings, matching_runs, person_privacy_holds) only. |

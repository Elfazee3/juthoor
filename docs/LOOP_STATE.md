...
| B6b | M0 follow-up: mask living-person birth_year + origin in `search_master_tree` results for non-privileged viewers (v2_3; the RPC already redacts private trees) (plan §5.5) | security, db, [LIVE-APPLY] | B1 | TODO | Add `is_privileged` (owner/approved, not public-only) to the accessible_flag CTE; wrap birth_year + origin_place_id + place names in `CASE WHEN is_accessible AND (is_privileged OR NOT is_person_living(person_id))`. Base it on the committed `20260424120510_search_master_tree_v2_2_drop_recreate.sql`. pgTAP: a public-tree living person's birth_year/origin come back NULL to a non-privileged searcher. |

...
| B8b | M1: `refresh_match_features(person_id)` + `_batch()` — derive the per-person feature vector (transliterate→normalize/phonetic; father/mother/grandparents matching `resolveParents`) + pgTAP parity (plan §6/§7-M1) | matching, db | B7, B8 | TODO | Populates `match_features`. Mirror `lib/tree/relationships.ts:resolveParents` M→father/F→mother/partner1-fallback incl. U/X + same-gender-partner. Uses transliterate_to_arabic before normalize_arabic/arabic_phonetic. Drains `match_features_dirty`. |

...
| B1 | M0: dump + commit ALL live-only DB objects into a migration; fresh `db reset` builds search/degrees end-to-end (brief §8-M0.1, plan §6 "hard dependency") | matching, db | S0 | IN_PROGRESS | DUMP COMPLETE + committed: all 16 live-only schema migrations faithfully reproduced (13 via subagent + 3 search_master_tree recovered after a session restart interrupted the subagent), byte-exact vs live `schema_migrations`. REMAINING: `db reset` build-verification — blocked, Docker went down after the session restart; relaunched & booting. Villages load via `seed_villages.sql` (not seed.sql); the 2 nakba_villages_seed migrations are intentionally NOT re-added (would duplicate that file). |
